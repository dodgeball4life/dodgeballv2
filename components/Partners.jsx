...
export default function Partners() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const offset1 = (scrollY * -0.5) % screenWidth;
      const offset2 = (scrollY * 0.3) % screenWidth;
      const offset3 = (scrollY * -0.2) % screenWidth;

      if (rowsRef.current[0])
        rowsRef.current[0].style.transform = `translateX(${offset1}px)`;
      if (rowsRef.current[1])
        rowsRef.current[1].style.transform = `translateX(${offset2}px)`;
      if (rowsRef.current[2])
        rowsRef.current[2].style.transform = `translateX(${offset3}px)`;
    };
  ...
  return (
    <div ref={sectionRef} className=" z-50">
      {/* Top Divider */}
      <div className="absolute top-[-10px] z-[100] left-0 w-full">
        <svg
          viewBox="0 0 1440 14"
      ...
      <section className={`${styles.parallaxSection}`}>
        <div className={styles.partnersTitle}>
          {"Partners".split("").map((letter, index) => (
            <span 
              key={index} 
              ...
              {Array(2)
                ...
                .map((img, j) => (
                  <img
                    ...
                    src={img.src}
                    alt={`Partner logo ${j}`}
                    className={`${styles.logoImage} ${img === logo5 ? styles.logoColored : ''}`}
                    draggable={false}
                    onError={(e) => {
